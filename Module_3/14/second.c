...
int main() {
    ...
    client_addr.sin_port = htons(FIRST_PORT);
    client_addr.sin_addr.s_addr = inet_addr(IP_ADDRESS);

    char str[BUF_SIZE];
    socklen_t addr_len = sizeof(client_addr); 
    printf("Клиент запущен. Введите сообщение:\n-> ");
    while (strcmp(buf, END_MSG) != 0) {
        input_string(str);
        if (sendto(sock_fd, str, BUF_SIZE, 0, (struct sockaddr*)&client_addr, addr_len) < 0) {
            perror("Ошибка отправки сообщения");
            close(sock_fd);
            exit(EXIT_FAILURE);
        }
        if (strcmp(str, END_MSG) == 0) break;
